...
{ 
    ...
    void Update()
    {
     if(Mathf.Abs(playerRigidbody.velocity.magnitude)<maxSpeed)
        ...
        if(Input.GetButtonDown("Jump") && groundcheck.GetComponent<FinalGroundCheck2>().amIOnTheGround==true)
        {
      ...
      playerAnimator.SetBool("amIGrounded",groundcheck.GetComponent<FinalGroundCheck2>().amIOnTheGround);   
    }
}
